...
function Home(props) {
  ...
  useEffect(async () => {
    const response = await fetch(
      "https://asm3-nodejs-backend.onrender.com/order/getAdminOrders"
    );
  ...
  useEffect(() => {
    const fetchClients = async () => {
      const response = await fetch(
        "https://asm3-nodejs-backend.onrender.com/users/getClients"
      );
      const responseData = await response.json();
      if (!response.ok) {
        alert(responseData.message);
      ...
      setClientNum(responseData.clientNum);
    };
    fetchClients();
  }, []);
  return (
                ...
                <div>
                  {history && (
                    <h2 className="text-dark mb-1 w-100 text-truncate font-weight-medium">
                      ...
                      {history.reduce((prevOrder, currentOrder) => {
                        return prevOrder + currentOrder.totalPrice;
                      }, 0)}
                    </h2>
                  )}
                  <h6 className="text-muted font-weight-normal mb-0 w-100 text-truncate">
